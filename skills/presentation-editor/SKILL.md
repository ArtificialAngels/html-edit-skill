...
| `saveLocal()` | 立即保存到浏览器本地（通常不必手动调用，已自动防抖保存） |
| `revertLocal()` | 丢弃本地保存，恢复到文件原始状态（会重载页面） |
...
编辑改动会**自动保存到浏览器 localStorage**，刷新页面后自动恢复，无需手动操作。

- **自动保存时机**：撤销栈变化、面板输入（防抖 800ms）、拖拽/缩放结束、页面隐藏或关闭前、编辑模式下每 15 秒兜底。
- **只存差异**：启动时记录内联样式与文本的「基线」，只持久化**真正被改过的**元素——存档体积小，且 HTML 源码更新后未被编辑的元素不会被旧存档覆盖。
- **存储键按页面身份隔离**：`pres-editor-autosave-v1:<fingerprint>`，fingerprint 由标题与页数哈希而来，同一浏览器打开不同 deck 不会串数据。
- **工具栏状态**：`自动保存：就绪 / 有未保存修改 / 已保存 HH:MM:SS / 已恢复 N 处修改 / 保存失败`。
- **「保存」按钮**：手动立即落盘。「还原」按钮：丢弃本地存档、回到文件原始状态。
- **产出文件仍用「导出HTML」**：localStorage 只是草稿层，要拿到可分发/可录屏的独立文件必须导出。

**两个注意点**：
1. `localStorage` 按站点隔离。用 `file://` 打开时，部分浏览器（Chrome）对每个本地文件视作不同源，或禁用本地存储——此时自动保存会提示「保存失败」，请改用「导出HTML」，或通过 HTTP 服务打开（`preview-server.js` / `启动预览服务器.bat`）。
2. 清理浏览器数据会连带清掉存档；重要版本请及时导出为文件。
...
16. **持久化只存差异，且基线不可重设**：自动保存必须对比「文件原始基线」，
    只记录真正改动过的元素。**恢复存档后绝不能重设基线**——否则周期保存会因
    「无差异」判定而把存档清空（表现为：第一次刷新能恢复，第二次就丢了）。

17. **路径要锚定分页容器**：元素定位路径以所属 `.step` 的序号为前缀，
    不要用 `document.body` 起点——`.step` 的 `active`/`build` 状态会变化，
    以 body 为起点会让路径漂移、恢复错位。同时不要把 `.step` 自身纳入存档。

18. **「还原」要先关闭自动保存**：清空存档后立即 `reload`，会触发
    `beforeunload` 把内存中的修改重新写回，导致还原失效。必须先置
    `autosaveEnabled = false` 并清掉待执行的防抖定时器。
